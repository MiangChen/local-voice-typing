...
- **按键即说，说完即粘**：一个全局快捷键开始/停止；识别结果**自动写入系统剪贴板**，无需手动复制。
- **本地 GPU 加速**：用 Whisper `large-v3`，中英文都很准；有 NVIDIA 显卡时近实时。
- **常驻不卡顿**：模型常驻显存，每次识别即时响应，无需反复加载。
- **顶栏状态图标**：系统托盘小图标实时显示状态——灰=空闲、蓝=聆听中、橙=识别中、绿 ✓=成功、红 !=失败；点击图标可看上次结果/报错、一键复制、打开仪表盘。无需盯着网页也能知道有没有录进去。
- **网页仪表盘**：`http://127.0.0.1:8765` 查看当前麦克风、录音状态、历史 100 条，每条一键复制。
